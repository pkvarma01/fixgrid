<?php
/**
 * FixGrid — Shared Initialiser (partials/init.php)
 *
 * Include ONCE at the top of every public page:
 *   require_once __DIR__ . '/partials/init.php';
 *
 * Provides:
 *  - h()     — HTML-safe escape (replaces h, ha, hc, h2, fh across pages)
 *  - parts() — pipe-split helper
 *  - $s      — settings array (DB-merged with defaults)
 *  - $navLinks, $services (parsed from $s)
 */
...
if (!function_exists('parts')) {
    function parts(string $s, string $sep = '|'): array {
        return array_values(array_filter(explode($sep, $s), 'strlen'));
    }
}

// ── Settings (DB-merged with defaults) ────────────────────────────────────
function getSettings(): array {
    ...
    try {
        ...
        $rows = $db->query(
            "SELECT setting_key, setting_value FROM app_settings
             WHERE setting_key LIKE 'hp_%'
                OR setting_key IN ('company_name','company_phone','company_email')"
    ...
    $defaults = [
        ...
        'hp_hero_stat1_num'  => '500+',  'hp_hero_stat1_lbl' => 'Happy clients',
        'hp_hero_stat2_num'  => '60 min','hp_hero_stat2_lbl' => 'Response',
        'hp_hero_stat3_num'  => '100%',  'hp_hero_stat3_lbl' => 'Verified',
        ...
        'hp_how_title'       => 'Book in Simple Steps',
        ...
        'hp_stat1_num'       => '500+',  'hp_stat1_lbl' => 'Clients Served',
        'hp_stat2_num'       => '60 min','hp_stat2_lbl' => 'Avg Response Time',
        'hp_stat3_num'       => '100%',  'hp_stat3_lbl' => 'Verified Professionals',
        'hp_stat4_num'       => '7-day', 'hp_stat4_lbl' => 'Service Guarantee',
        ...
        'hp_zones_list'      => 'Cyber City|green|Gurgaon|MG Road|green|Gurgaon|Sohna Road|green|Gurgaon|Golf Course Rd|green|Gurgaon|Udyog Vihar|green|Gurgaon|Sector 14-17|green|Gurgaon|South City|green|Gurgaon|Manesar|green|Gurgaon|Aurangabad|yellow|Bihar|Patna|yellow|Bihar',
        ...
        'hp_nav_links'       => 'Services|services.php|How it Works|index.php#how|Areas|index.php#zones|Corporate|index.php#corporate|About|about.php',
        'hp_nav_cta'         => '⚡ Book Now',
        ...
        'company_name'       => 'FixGrid',
    ];

    return array_merge($defaults, $rows);
}

// ── Populate globals used by navbar & footer partials ─────────────────────
$s = getSettings();

// Nav links (parsed from settings so admin can update them)
$navParts = parts($s['hp_nav_links']);
$navLinks = [];
for ($i = 0; $i + 1 < count($navParts); $i += 2) {
    $navLinks[] = ['label' => $navParts[$i], 'href' => $navParts[$i + 1]];
}

// Services list (parsed — shared by navbar booking form and footer)
$svParts  = parts($s['hp_services_list']);
$services = [];
for ($i = 0; $i + 2 < count($svParts); $i += 3) {
    $services[] = ['name' => $svParts[$i], 'icon' => $svParts[$i + 1], 'price' => $svParts[$i + 2]];
}
